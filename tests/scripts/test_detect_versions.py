...
def test_dind_detects_static_pseudo_version(tmp_path):
	detector = make_detector(tmp_path, 'dind')
	assert detector.detect_versions() == ['dind']
	assert detector.latest_version == 'dind'
...
def test_load_past_detected_versions_reads_existing_file(tmp_path):
	...
	output_path.write_text(yaml.dump({'detected_versions': {'python': ['3.14', '3.13']}}))
...
def test_detect_docker_image_incomplete_constraints_stops_after_first(
...
def test_detect_docker_image_no_docker_image_for_unknown_package_returns_past(
	tmp_path, monkeypatch, capsys
):
...
def test_detect_node_versions_non_list_response_returns_empty_no_fallback(
	tmp_path, monkeypatch, capsys
):
