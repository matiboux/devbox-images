...
def test_generate_build_matrix_python_base_variants(tmp_path):
    # NOTE: locks in current behavior. The base-variant tag component is
    # generated without the 'always' unlabeled flag, so the variant name is
    # currently emitted as "python_variant<value>" (e.g. "python_variantslim")
    # instead of the documented bare "slim"/"alpine" suffix in README.md.
    bm = make_matrix(
        tmp_path,
        ["python"],
        {"python": ["3.14.6"]},
