...
: > "${CURL_LOG}"
output=$(CURL_STUB_LOG="${CURL_LOG}" sh "${SCRIPT}" '0.13.1' 2>&1)
code=$?
...
: > "${CURL_LOG}"
output=$(CURL_STUB_LOG="${CURL_LOG}" sh "${SCRIPT}" '0.13.1' 2>&1)
assert_contains "$(cat "${CURL_LOG}")" 'dive_0.13.1_linux_arm64.tar.gz'
rm -f "${STUB_BIN_DIR}/uname"
...
assert_contains "${output}" 'Installed dive version 0.13.1 to /usr/local/bin/dive.'
rm -f "${STUB_BIN_DIR}/uname"

test_case "'latest' (default) resolves the tag_name from the GitHub releases API"
stub_cmd uname 'echo x86_64'
output=$(CURL_STUB_API_BODY='{"tag_name":"v0.13.1"}' sh "${SCRIPT}" 2>&1)
code=$?
assert_exit_code "${code}" 0 "${output}"
assert_contains "${output}" 'Installed dive version 0.13.1 to /usr/local/bin/dive.'
rm -f "${STUB_BIN_DIR}/uname"
...
stub_cmd uname 'echo x86_64'
output=$(CURL_STUB_API_BODY='[{"ref":"refs/tags/v0.13.1"},{"ref":"refs/tags/v0.13.9"}]' sh "${SCRIPT}" '0.13' 2>&1)
code=$?
assert_exit_code "${code}" 0 "${output}"
assert_contains "${output}" 'Installed dive version 0.13.9 to /usr/local/bin/dive.'
rm -f "${STUB_BIN_DIR}/uname"
...
code=$?
assert_exit_code "${code}" 1
assert_contains "${output}" "Version '99.99' not found in dive repository."
rm -f "${STUB_BIN_DIR}/uname"
...
: > "${STUB_BIN_DIR}/mv.log"
output=$(sh "${SCRIPT}" '0.13.1' 2>&1)
code=$?
assert_exit_code "${code}" 1
assert_contains "${output}" 'Failed to extract dive binary from archive.'
...
code=$?
assert_exit_code "${code}" 1
assert_contains "${output}" 'Failed to install dive binary in /usr/local/bin.'
stub_cmd_logging mv
rm -f "${STUB_BIN_DIR}/uname"
