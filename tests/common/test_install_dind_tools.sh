#!/bin/sh
# Tests for src/common/install-dind-tools.sh
#
# Like tests/common/test_install_docker_tools.sh, this copies the real
...
# install-dind.sh/install-lazydocker.sh/etc.

. "$(dirname "$0")/../support/shell/harness.sh"

REAL_SCRIPT="${COMMON_DIR}/install-dind-tools.sh"

FAKE_ROOT="$(mktemp -d)"
mkdir -p "${FAKE_ROOT}/src/common"
cp "${REAL_SCRIPT}" "${FAKE_ROOT}/src/common/install-dind-tools.sh"

CALL_LOG="${FAKE_ROOT}/calls.log"

for name in install-dind.sh install-lazydocker.sh install-hadolint.sh \
...
SCRIPT="${FAKE_ROOT}/src/common/install-dind-tools.sh"

test_case 'installs a Docker Engine and every companion dev tool, defaulting their versions to empty'
: > "${CALL_LOG}"
output=$(sh "${SCRIPT}" 2>&1)
code=$?
assert_exit_code "${code}" 0 "${output}"
calls="$(cat "${CALL_LOG}")"
assert_contains "${calls}" 'install-dind.sh'
...
: > "${CALL_LOG}"
cat > "${FAKE_ROOT}/src/common/install-dind.sh" <<EOF
#!/bin/sh
echo "install-dind.sh \$*" >> '${CALL_LOG}'
exit 1
EOF
chmod +x "${FAKE_ROOT}/src/common/install-dind.sh"
output=$(sh "${SCRIPT}" 2>&1)
code=$?
