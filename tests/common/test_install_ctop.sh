...
: > "${CURL_LOG}"
output=$(CURL_STUB_LOG="${CURL_LOG}" sh "${SCRIPT}" '0.7.7' 2>&1)
code=$?
...
: > "${CURL_LOG}"
output=$(CURL_STUB_LOG="${CURL_LOG}" sh "${SCRIPT}" '0.7.7' 2>&1)
assert_contains "$(cat "${CURL_LOG}")" 'ctop-0.7.7-linux-arm64'
rm -f "${STUB_BIN_DIR}/uname"
...
assert_contains "${output}" 'Installed ctop version 0.7.7 to /usr/local/bin/ctop.'
rm -f "${STUB_BIN_DIR}/uname"

test_case "'latest' (default) resolves the tag_name from the GitHub releases API"
stub_cmd uname 'echo x86_64'
output=$(CURL_STUB_API_BODY='{"tag_name":"v0.7.7"}' sh "${SCRIPT}" 2>&1)
code=$?
assert_exit_code "${code}" 0 "${output}"
assert_contains "${output}" 'Installed ctop version 0.7.7 to /usr/local/bin/ctop.'
rm -f "${STUB_BIN_DIR}/uname"
...
stub_cmd uname 'echo x86_64'
output=$(CURL_STUB_API_BODY='[{"ref":"refs/tags/v0.7.7"},{"ref":"refs/tags/v0.7.10"}]' sh "${SCRIPT}" '0.7' 2>&1)
code=$?
assert_exit_code "${code}" 0 "${output}"
assert_contains "${output}" 'Installed ctop version 0.7.10 to /usr/local/bin/ctop.'
rm -f "${STUB_BIN_DIR}/uname"
...
code=$?
assert_exit_code "${code}" 1
assert_contains "${output}" "Version '99.99' not found in ctop repository."
rm -f "${STUB_BIN_DIR}/uname"

test_case 'an install failure into /usr/local/bin is reported'
stub_cmd uname 'echo x86_64'
stub_cmd_logging install 1
output=$(sh "${SCRIPT}" '0.7.7' 2>&1)
code=$?
assert_exit_code "${code}" 1
assert_contains "${output}" 'Failed to install ctop binary in /usr/local/bin.'
stub_cmd_logging install
rm -f "${STUB_BIN_DIR}/uname"
