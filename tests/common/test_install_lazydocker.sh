#!/bin/sh
# Tests for src/common/install-lazydocker.sh
#
# `tar` and `mv` are stubbed as safe no-ops: this script's final step is an
# unconditional `mv .../lazydocker /usr/local/bin/lazydocker`, a real
# hardcoded system path with no override, so we must never let it actually
# run un-intercepted.

. "$(dirname "$0")/../support/shell/harness.sh"

SCRIPT="${COMMON_DIR}/install-lazydocker.sh"
...
: > "${CURL_LOG}"
output=$(CURL_STUB_LOG="${CURL_LOG}" sh "${SCRIPT}" '0.23.3' 2>&1)
code=$?
...
: > "${CURL_LOG}"
output=$(CURL_STUB_LOG="${CURL_LOG}" sh "${SCRIPT}" '0.23.3' 2>&1)
assert_contains "$(cat "${CURL_LOG}")" 'lazydocker_0.23.3_Linux_arm64.tar.gz'
rm -f "${STUB_BIN_DIR}/uname"
...
assert_contains "${output}" 'Installed lazydocker version 0.23.3 to /usr/local/bin/lazydocker.'
rm -f "${STUB_BIN_DIR}/uname"

test_case "'latest' (default) resolves the tag_name from the GitHub releases API"
stub_cmd uname 'echo x86_64'
output=$(CURL_STUB_API_BODY='{"tag_name":"v0.23.3"}' sh "${SCRIPT}" 2>&1)
code=$?
assert_exit_code "${code}" 0 "${output}"
assert_contains "${output}" 'Installed lazydocker version 0.23.3 to /usr/local/bin/lazydocker.'
rm -f "${STUB_BIN_DIR}/uname"
...
stub_cmd uname 'echo x86_64'
output=$(CURL_STUB_API_BODY='[{"ref":"refs/tags/v0.23.3"},{"ref":"refs/tags/v0.23.10"}]' sh "${SCRIPT}" '0.23' 2>&1)
code=$?
assert_exit_code "${code}" 0 "${output}"
assert_contains "${output}" 'Installed lazydocker version 0.23.10 to /usr/local/bin/lazydocker.'
rm -f "${STUB_BIN_DIR}/uname"
...
code=$?
assert_exit_code "${code}" 1
assert_contains "${output}" "Version '99.99' not found in lazydocker repository."
rm -f "${STUB_BIN_DIR}/uname"
...
: > "${STUB_BIN_DIR}/mv.log"
output=$(sh "${SCRIPT}" '0.23.3' 2>&1)
code=$?
assert_exit_code "${code}" 1
assert_contains "${output}" 'Failed to extract lazydocker binary from archive.'
...
code=$?
assert_exit_code "${code}" 1
assert_contains "${output}" 'Failed to install lazydocker binary in /usr/local/bin.'
stub_cmd_logging mv
rm -f "${STUB_BIN_DIR}/uname"
