#!/bin/sh
# Tests for src/common/install-gh.sh
#
# `tar` and `mv` are stubbed as safe no-ops: this script's final step is an
# unconditional `mv .../gh /usr/local/bin/gh`, a real hardcoded system path
# with no override, so we must never let it actually run un-intercepted.

. "$(dirname "$0")/../support/shell/harness.sh"

SCRIPT="${COMMON_DIR}/install-gh.sh"
...
: > "${CURL_LOG}"
output=$(CURL_STUB_LOG="${CURL_LOG}" sh "${SCRIPT}" '2.63.2' 2>&1)
code=$?
...
: > "${CURL_LOG}"
output=$(CURL_STUB_LOG="${CURL_LOG}" sh "${SCRIPT}" '2.63.2' 2>&1)
assert_contains "$(cat "${CURL_LOG}")" 'gh_2.63.2_linux_arm64.tar.gz'
rm -f "${STUB_BIN_DIR}/uname"
...
assert_contains "${output}" 'Installed gh version 2.63.2 to /usr/local/bin/gh.'
rm -f "${STUB_BIN_DIR}/uname"

test_case "'latest' (default) resolves the tag_name from the GitHub releases API"
stub_cmd uname 'echo x86_64'
output=$(CURL_STUB_API_BODY='{"tag_name":"v2.63.2"}' sh "${SCRIPT}" 2>&1)
code=$?
assert_exit_code "${code}" 0 "${output}"
assert_contains "${output}" 'Installed gh version 2.63.2 to /usr/local/bin/gh.'
rm -f "${STUB_BIN_DIR}/uname"
...
stub_cmd uname 'echo x86_64'
output=$(CURL_STUB_API_BODY='[{"ref":"refs/tags/v2.63.2"},{"ref":"refs/tags/v2.63.10"}]' sh "${SCRIPT}" '2.63' 2>&1)
code=$?
assert_exit_code "${code}" 0 "${output}"
assert_contains "${output}" 'Installed gh version 2.63.10 to /usr/local/bin/gh.'
rm -f "${STUB_BIN_DIR}/uname"
...
code=$?
assert_exit_code "${code}" 1
assert_contains "${output}" "Version '99.99' not found in gh repository."
rm -f "${STUB_BIN_DIR}/uname"
...
: > "${STUB_BIN_DIR}/mv.log"
output=$(sh "${SCRIPT}" '2.63.2' 2>&1)
code=$?
assert_exit_code "${code}" 1
assert_contains "${output}" 'Failed to extract gh binary from archive.'
...
code=$?
assert_exit_code "${code}" 1
assert_contains "${output}" 'Failed to install gh binary in /usr/local/bin.'
stub_cmd_logging mv
rm -f "${STUB_BIN_DIR}/uname"
